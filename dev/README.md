...
### parse-cwl.ts
```
ts-node parse-cwl.ts both|all|ess|essential|expl3|<package>.cwl
```
...
Further, `expl3.cwl` and `latex2e-expl3.cwl` files generated by `latex3command.py` are also parsed to `expl3.json` and `latex2e-expl3.cwl` and placed at `../data/packages`.
